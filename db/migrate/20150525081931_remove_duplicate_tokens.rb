class RemoveDuplicateTokens < ActiveRecord::Migration
  def up
    ...
    Token.where(:id => Token.joins("left outer join hosts on hosts.id = tokens.host_id").
                             where("hosts.type != 'Host::Managed'").pluck(:id)).delete_all
    ...
    if existing_tokens.empty?
      Token.where(:id => Token.where(:host_id => hosts_with_duplicate_tokens).pluck(:id)).delete_all
    else
      Token.where(:id => Token.where('host_id in (?) and id not in (?)', hosts_with_duplicate_tokens, existing_tokens).pluck(:id)).delete_all
    end
