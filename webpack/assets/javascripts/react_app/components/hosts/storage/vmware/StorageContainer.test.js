...
describe('StorageContainer', () => {
  ...
  it('adds a disk when button is clicked', () => {
    expect(wrapper.render().find('.disk-container')).toHaveLength(1);

    wrapper.find('button.btn-add-disk').simulate('click');

    expect(wrapper.render().find('.disk-container')).toHaveLength(2);
  });

  it('adds a controller when button is clicked', () => {
    expect(wrapper.render().find('.controller-container')).toHaveLength(1);

    wrapper.find('button.btn-add-controller').simulate('click');

    expect(wrapper.render().find('.controller-container')).toHaveLength(2);
  });
  ...
  it('doesnt remove controller when none are selected', () => {
    ...
    expect(wrapper.render().find('.controller-container')).toHaveLength(0);
  });
