...
import {
  ...
  selectHostStatusOkTotalPath,
  selectHostStatusOkOwnedPath,
  selectHostStatusWarnTotalPath,
  selectHostStatusWarnOwnedPath,
  selectHostStatusErrorTotalPath,
  selectHostStatusErrorOwnedPath,
...
const Status = ({ name }) => {
  ...
  const okTotalPath = useSelector(state =>
    selectHostStatusOkTotalPath(state, name)
  );
  const okOwnedPath = useSelector(state =>
    selectHostStatusOkOwnedPath(state, name)
  );
  const warnTotalPath = useSelector(state =>
    selectHostStatusWarnTotalPath(state, name)
  );
  const warnOwnedPath = useSelector(state =>
    selectHostStatusWarnOwnedPath(state, name)
  );
  const errorTotalPath = useSelector(state =>
    selectHostStatusErrorTotalPath(state, name)
  );
  const errorOwnedPath = useSelector(state =>
    selectHostStatusErrorOwnedPath(state, name)
  );
  const {
  ...
  return (
    <GridItem sm={12} xl2={6}>
      <Card className={cardClass} isExpanded={expanded} isHoverable>
        ...
        <CardExpandableContent>
          {details.length > 0 ? (
            <Details data={details.asMutable()} />
          ) : (
            __('Nothing to show')
          )}
        </CardExpandableContent>
