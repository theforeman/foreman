...
export const selectCurrentLocation = state =>
  selectLayout(state).currentLocation;
export const selectCurrentOrganization = state =>
  selectLayout(state).currentOrganization;
...
const patternflyItems = (
  ...
  data.forEach(item => {
    ...
    item.children.forEach(child => {
      if (isEmpty(activeMenu) && child.url === activePath) {
        // activeMenu after Full page reload
        activeFlag = true;
