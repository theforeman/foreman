class Puppetclass < Apipie::Client::CliCommand

  desc 'index', 'List all puppetclasses.'
  ...
  desc 'create', 'Create a puppetclass.'
  method_option :name, :required => true, :desc => '', :type => :string
  def create
    params = transform_options([], {"puppetclass"=>["name"]})
    data, resp = client.create(params)
    print_data(data)
  end

  desc 'update', 'Update a puppetclass.'
  method_option :id, :required => 'true'
  method_option :id, :required => true, :desc => '', :type => :string
  method_option :name, :required => true, :desc => '', :type => :string
  def update
    params = transform_options(["id"], {"puppetclass"=>["name"]})
    data, resp = client.update(params)
    print_data(data)
  end

  desc 'destroy', 'Delete a puppetclass.'
  method_option :id, :required => 'true'
  method_option :id, :required => true, :desc => '', :type => :string
  def destroy
    params = transform_options(["id"])
