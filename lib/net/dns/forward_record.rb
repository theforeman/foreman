module Net
  module DNS
    class ForwardRecord < DNS::Record
      ...
      def conflicts
        @conflicts ||= [dns_lookup(hostname, :ipversion => self.ipversion)].delete_if { |c| c == self }.compact
      end

      # Verifies that a record already exists on the dns server
      def valid?
        self == dns_lookup(hostname, :ipversion => self.ipversion)
      end

      def ptr
        dns_lookup(ip, :ipversion => self.ipversion)
      end
