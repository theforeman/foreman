class Setting::Puppet < Setting
  ...
  def self.default_settings
    [
      ...
      set('default_puppet_environment', N_("Foreman will default to this puppet environment if it cannot auto detect one"), "production", N_('Default Puppet environment'), nil, { :collection => Proc.new { Hash[Environment.all.map { |env| [env[:name], env[:name]] }] } }),
      set('puppetrun', N_("Enable puppetrun support"), false, N_('Puppetrun')),
      set('Default_variables_Lookup_Path', N_("Foreman will evaluate host smart variables in this order by default"), ["fqdn", "hostgroup", "os", "domain"], N_('Default variables lookup path')),
      ...
      set('update_subnets_from_facts', N_("Foreman will update a host's subnet from its facts"), 'none', N_('Update subnets from facts'), nil, { :collection => Proc.new { update_subnets_from_facts_types } }),
      ...
      set('default_location', N_("Hosts created after a puppet run that did not send a location fact will be placed in this location"), '', N_('Default location'), nil, { :collection => Proc.new { Hash[Location.all.map { |loc| [loc[:title], loc[:title]] }] } }),
      set('default_organization', N_("Hosts created after a puppet run that did not send a organization fact will be placed in this organization"), '', N_('Default organization'), nil, {:collection => Proc.new { Hash[Organization.all.map { |org| [org[:title], org[:title]] }] } }),
