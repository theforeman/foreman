  ...
  def self.load_defaults
    ...
    self.transaction do
      [
        ...
        self.set('Default_variables_Lookup_Path', N_("Foreman will evaluate host smart-variables in this order"), ["fqdn", "hostgroup", "os", "domain"]),
        ...
        self.set('interpolate_erb_in_parameters', N_("Foreman will expose variables (theforeman/common_parammeters) on the ENC output"), true),
        self.set('enc_environment', N_("Foreman will explicitely set the puppet environment in the ENC yaml output. This will avoid conflicts between the environment in puppet.conf and the environment set in Foreman"), true),
        self.set('use_uuid_for_certificates', N_("Foreman will use random UUID's for certificate signing instead of hostnames"), false),
        self.set('update_environment_from_facts', N_("Foreman will update a host's environment from its facts"), false),
        self.set('remove_classes_not_in_environment', N_("Remove classes from enviroments different to the host's environment when a host and its host group have different environments"), false),
        self.set('host_group_matchers_inheritance', N_("Foreman host group matchers will be inherited by children to set puppet classes parameters values"), true),
