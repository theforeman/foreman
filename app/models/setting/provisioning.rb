...
class Setting::Provisioning < Setting
  ...
  def self.load_defaults
    ...
    self.transaction do
      [
        self.set('root_pass', N_("Default encrypted root password on provisioned systems (default is 123123)"), "$1$default$hCkak1kaJPQILNmYbUXhD0"),
        self.set('unattended_url', N_("The URL that systems will retrieve templates from during build (normally http as many installers don't support https)"), "http://#{Facter.fqdn}"),
        ...
        self.set('manage_puppetca', N_("Should Foreman automate certificate signing upon provisioning new system"), true),
        ...
        self.set('update_ip_from_built_request', N_("Should we use the originating IP of the built request to update the system's IP?"), false),
        self.set('use_shortname_for_vms', N_("Should Foreman use the short system name instead of the FQDN for creating new virtual machines"), false)
