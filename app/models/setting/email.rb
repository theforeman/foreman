class Setting::Email < Setting
  ...
  def self.default_settings
    ...
    [
      ...
      self.set('smtp_openssl_verify_mode', N_("When using TLS, you can set how OpenSSL checks the certificate"), nil, N_('SMTP OpenSSL verify mode'), nil, { :collection => Proc.new {{'' => _("Default verification mode"), :none => _("none"), :peer => "peer", :client_once => "client_once", :fail_if_no_peer_cert => "fail_if_no_peer_cert"}}}),
      self.set('smtp_address', N_("Address to connect to"), nil, N_('SMTP address')),
      self.set('smtp_port', N_("Port to connect to"), 25, N_('SMTP port')),
      self.set('smtp_domain', N_("HELO/EHLO domain"), nil, N_('SMTP HELO/EHLO domain')),
      self.set('smtp_user_name', N_("Username to use to authenticate, if required"), nil, N_('SMTP username')),
      self.set('smtp_password', N_("Password to use to authenticate, if required"), nil, N_('SMTP password'), nil, {:encrypted => true}),
      self.set('smtp_authentication', N_("Specify authentication type, if required"), nil, N_('SMTP authentication'), nil, { :collection => Proc.new {{:plain => "plain", :login => "login", :cram_md5 => "cram_md5", '' => _("none")}}}),
