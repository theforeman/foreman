class SmartProxy < ActiveRecord::Base
  ...
  def self.smart_proxy_ids_for(hosts)
    ...
    ids << hosts.joins(:puppet_aspect).pluck('DISTINCT puppet_aspects.puppet_proxy_id')
    ids << hosts.joins(:puppet_aspect).pluck('DISTINCT puppet_aspects.puppet_ca_proxy_id')
  ...
  def taxonomy_foreign_conditions
    ...
    hosts = PuppetAspect.where(conditions).pluck(:host_id)
    {:id => hosts}
  end
