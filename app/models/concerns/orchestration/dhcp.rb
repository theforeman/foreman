module Orchestration::DHCP
  ...
  def select_attribute(interface)
    return interface.mac if interface.mac
    return interface.identifier if interface.identifier
    return interface.id
  end

  def queue_dhcp_create
    logger.debug "Scheduling new DHCP reservations for #{self}"
    queue.create(id: "dhcp_create_#{select_attribute(self)}", name: _("Create DHCP Settings for %s") % self, priority: 10, action: [self, :set_dhcp]) if dhcp?
  end

  def queue_dhcp_update
    return unless dhcp_update_required?
    logger.debug("Detected a changed required for DHCP record")
    queue.create(id: "dhcp_remove_#{select_attribute(old)}", name: _("Remove DHCP Settings for %s") % old, priority: 5, action: [old, :del_dhcp]) if old.dhcp?
    queue.create(id: "dhcp_create_#{select_attribute(self)}", name: _("Create DHCP Settings for %s") % self, priority: 9, action: [self, :set_dhcp]) if dhcp?
  end
  ...
  def queue_dhcp_destroy
    ...
    queue.create(id: "dhcp_remove_#{select_attribute(self)}", name: _("Remove DHCP Settings for %s") % self, priority: 5, action: [self, :del_dhcp])
    true
  end
  ...
  def queue_remove_dhcp_conflicts
    ...
    queue.create(id: "dhcp_conflicts_remove_#{select_attribute(self)}", name: _("DHCP conflicts removal for %s") % self, priority: 5, action: [self, :del_dhcp_conflicts])
  end
