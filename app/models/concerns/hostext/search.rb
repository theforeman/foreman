  ...
  module Search
    ...
    module ClassMethods
      ...
      def search_by_puppetclass(key, operator, value)
        ...
        host_ids         = Host.authorized(:view_hosts, Host).where(conditions).joins(:puppetclasses).uniq.pluck(:id)
      ...
      def search_by_config_group(key, operator, value)
        ...
        host_ids      = Host::Managed.authorized(:view_hosts, Host).where(conditions).joins(:config_groups).uniq.pluck(:id)
