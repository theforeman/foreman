...
class Setting < ApplicationRecord
  ...
  TYPES= %w{integer boolean hash array string}.freeze
  FROZEN_ATTRS = %w{name category full_name}.freeze
  NONZERO_ATTRS = %w{puppet_interval idle_timeout entries_per_page max_trend outofsync_interval}.freeze
  BLANK_ATTRS = %w{ host_owner trusted_hosts login_delegation_logout_url authorize_login_delegation_auth_source_user_autocreate root_pass default_location default_organization websockets_ssl_key websockets_ssl_cert oauth_consumer_key oauth_consumer_secret login_text
                    smtp_address smtp_domain smtp_user_name smtp_password smtp_openssl_verify_mode smtp_authentication sendmail_arguments sendmail_location http_proxy http_proxy_except_list}.freeze
  ARRAY_HOSTNAMES = %w{trusted_hosts}.freeze
  URI_ATTRS = %w{foreman_url unattended_url}.freeze
  URI_BLANK_ATTRS = %w{login_delegation_logout_url}.freeze
  IP_ATTRS = %w{libvirt_default_console_address}.freeze
  REGEXP_ATTRS = %w{remote_addr}.freeze
  EMAIL_ATTRS = %w{administrator email_reply_address}.freeze
  NOT_STRIPPED = %w{}.freeze
