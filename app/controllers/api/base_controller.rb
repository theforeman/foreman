      ...
      params.keys.each do |param|
        if param =~ /(\w+)_id$/
          if allowed_nested_id.include?(param)
            ...
            # log errors if parameter 'xyz_id' was passed manually in URL string rather than generated by nested route.
            logger.info "WARNING: #{param} is not allowed as nested parameter for #{controller_name.pluralize}. Allowed parameters are #{allowed_nested_id.join(', ')}"
            logger.error "ERROR: Array 'allowed_nested_id' is not defined for #{controller_name.pluralize}_controller" if allowed_nested_id.empty?
          end
        end
      end
    end
