module Api
  ...
  class BaseController < ActionController::Base
    ...
    def authorized_scope(resource, klass)
      scope = klass.scoped
      scope.authorized("#{action_permission}_#{resource}", klass)
    end
    ...
    def find_resource(controller = controller_name, scope = nil)
    ...
    def set_nested_resource(resource, name)
      if resource
        return instance_variable_set(:"@#{name}", resource)
      else
        ...
        if md = param.match(/(\w+)_id$/)
          if allowed_nested_id.include?(param)
            ...
            resource = resource_identifying_attributes.find do |key|
              ...
              scope = authorized_scope("#{action_permission}_#{controller}", model)

              if scope.respond_to?(find_method)
                (resource = scope.send(find_method, params[param])) and break resource
              end
            end

            @nested_obj ||= resource
            set_nested_resource(resource, model_name)
          else
    ...
    def get_class(model_string)
      model_string.classify.constantize
    end
    ...
    def not_found_if_nested_id_exists
      ...
      allowed_nested_id.each do |obj_id|
        if params[obj_id].present?
          messages << "#{obj_id.humanize} not found by id '#{params[obj_id]}'"
        end
      end

      not_found(:message => messages) and return false if !messages.empty?
    end
