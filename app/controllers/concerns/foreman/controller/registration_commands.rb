module Foreman::Controller::RegistrationCommands
  ...
  def command
    ...
    "set -o pipefail && #{utility[:download]} #{insecure} '#{registration_url(@smart_proxy)}#{args_query if args_query != '?'}' #{command_headers} | bash"
  end
  ...
  def utility
    download_utility(registration_params['download_utility'])
  end
