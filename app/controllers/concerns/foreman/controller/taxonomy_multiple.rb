module Foreman::Controller::TaxonomyMultiple
  ...
  included do
    #TODO: make this before filter work, its not working as the same filter is defined in the systems controller
  ...
  def update_multiple_taxonomies type
    ...
    if params[type][:optimistic_import] == 'yes'
      @systems.update_all("#{type}_id".to_sym => taxonomy.id)
      #systems location needs to be updated before import missing ids
    ...
    notice "Updated systems: Changed #{type.to_s.classify}"
    redirect_back_or_to systems_path
  end
