module Foreman
  class Deprecation
    ...
    def self.deprecation_warning(foreman_version_deadline, info)
      ...
      ActiveSupport::Deprecation.warn("You are using a deprecated behavior, it will be removed in version #{foreman_version_deadline}, #{info}", caller(2)) unless Rails.env.test?
    end

    def self.api_deprecation_warning(info)
      ActiveSupport::Deprecation.warn("Your API call uses deprecated behavior, #{info}", caller) unless Rails.env.test?
    end
  end
end
