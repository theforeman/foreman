module ImagesHelper
  def image_field(f, opts = {})
    ...
    lazy_load = opts[:lazy_load] || false
    if lazy_load
      selectable_f(f, :uuid, [],
        {:include_blank => _('Loading images...') },
        :label => _('Image'),
        :disabled => true,
        :indicator => true,
        :help_inline => opts[:help_inline] || _("Image ID as provided by the compute resource, e.g. ami-.."),
        :data => {
          :url => available_images_compute_resource_path(@compute_resource),
          :selected => f.object.uuid || nil
        })
    else
