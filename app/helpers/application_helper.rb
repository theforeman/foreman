...
module ApplicationHelper
  ...
  def filter_turbolinks(args, &block)
    block_given? ? index = 0 : index = 1
    # binding.pry
    case args[index]
    when Hash
      disable_turbolink(args) if DisableTurbolinks.include? args[index]
    when String
      disable_turbolink(args) if DisableTurbolinks.has_path? args[index]
    when :back
      return
    when ActiveRecord::Base, Array
      disable_turbolink(args) if DisableTurbolinks.has_path?(url_for args[index])
    else
      raise "Unknown link format: #{args[index]}.class"
    end
  end

  def disable_turbolink(args)
    if args[2]
      args[2][:data] ? args[2][:data].merge!({ :'no-turbolink' => "" }) : args[2][:data] = { :'no-turbolink' => "" }
    else
      args << { :data => { :'no-turbolink' => "" }}
    end
  end
