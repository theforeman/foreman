    ...
    name       = name.split(FactName::SEPARATOR).map do |current_name|
      ...
      if parent.present? && h(parent.name) == memo
        ...
        if value_name != memo || value.compose
          parameters = {:parent_fact => memo} #params.select { |k,i| k.to_s == 'host_id' }.merge(:parent_fact => memo)
          if (params[:host_id]) 
            parameters.merge!({:host_id => params[:host_id]}) 
            url=host_facts_path(parameters)
          else
