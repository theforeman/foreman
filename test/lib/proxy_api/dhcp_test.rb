...
class ProxyApiDhcpTest < ActiveSupport::TestCase
  ...
  test "subnet_usage should get the subnet usage using only the network address" do
    subnet_mock = mock('subnet')
    subnet_mock.expects(:network).returns('192.168.0.0')
    subnet_mock.expects(:from).returns(nil)

    @dhcp.expects(:get).with('192.168.0.0/usage').returns(fake_response({:free => 246, :size => 254, :used => 8}))
    assert_equal({'free' => 246, 'size' => 254, 'used' => 8}, @dhcp.subnet_usage(subnet_mock))
  end

  test "subnet_usage should get the subnet usage using the network, from and to addresses" do
    from_mock = mock('from')
    from_mock.expects(:to_s).returns('192.168.0.50')
    from_mock.expects(:present?).returns(true)
    to_mock = mock('to')
    ...
    @dhcp.expects(:get).with() do |path|
      # Params built with a hash, so order can change
      path.include?('192.168.0.0/usage?') &&
        path.include?('from=192.168.0.50') &&
        path.include?('to=192.168.0.150')
    end.returns(fake_response({:free => 80, :size => 100, :used => 20}))
    assert_equal({'free' => 80, 'size' => 100, 'used' => 20}, @dhcp.subnet_usage(subnet_mock))
  end
end
