...
class SmartProxyTest < ActiveSupport::TestCase
  ...
  test "can search smart proxy by pool" do
    proxy = FactoryBot.create(:puppet_and_ca_smart_proxy)
    results = SmartProxy.search_for("pool = #{proxy.pools.first.name}")
    assert_equal 1, results.count
    assert_includes results, proxy
    results = SmartProxy.search_for("pool = #{proxy.pools.first.name}-empty")
    assert_equal 0, results.count
  end
