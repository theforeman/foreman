...
class EmailSettingTest < ActiveSupport::TestCase
  test 'delivery_settings return values set by Setting#[]=' do
    ...
    assert_equal({"address" => "sm@example.com", "authentication" => "plain", "enable_starttls_auto" => true, "port" => 25}, ApplicationMailer.delivery_settings)
  end

  test 'setting should be clear from delivery settings when it empty' do
    Setting[:smtp_address] = "sm@example.com"
    Setting[:smtp_authentication] = ''
    assert_equal({"address" => "sm@example.com", "enable_starttls_auto" => true, "port" => 25}, ApplicationMailer.delivery_settings)
  end
