...
class FilterTest < ActiveSupport::TestCase
  test "#unlimited?" do
    f = FactoryBot.build :filter
    assert_nil f.search, 'default filter is not unlimited'
    assert f.unlimited?
  end

  test "#limited?" do
    f = FactoryBot.build :filter, :on_name_all
  ...
  test "#resource_class" do
    f = FactoryBot.build(:filter, :resource_type => 'Bookmark')
  ...
  test "#granular? for unknown resource type" do
    f = FactoryBot.build(:filter, :resource_type => 'BookmarkThatDoesNotExist')
    refute f.granular?
  end
  ...
  test "unlimited filters have nilified search string" do
    f = FactoryBot.build(:filter, :search => 'name ~ a*', :unlimited => '1')
    assert f.valid?
    assert_nil f.search

    f = FactoryBot.build(:filter, :search => '', :unlimited => '1')
    assert f.valid?
    assert_nil f.search

    f = FactoryBot.build(:filter, :search => 'name ~ a*', :unlimited => '0')
    assert f.valid?
  ...
  context 'with taxnomies' do
    ...
    test "filter with organization set is always limited before validation" do
      f = FactoryBot.build(:filter, :search => '', :unlimited => '1', :organization_ids => [@organization.id])
      assert f.valid?
      assert f.limited?
    ...
    test "filter with location set is always limited before validation" do
      f = FactoryBot.build(:filter, :search => '', :unlimited => '1', :location_ids => [@location.id])
      assert f.valid?
      assert f.limited?
    ...
    test "taxonomies can be assigned only if resource allows it" do
      fb = FactoryBot.build(:filter, :resource_type => 'Bookmark', :organization_ids => [@organization.id])
      fd = FactoryBot.build(:filter, :resource_type => 'Domain', :organization_ids => [@organization.id])
      refute_valid fb
      assert_valid fd
  ...
  test "filter remains unlimited when no organization assigned" do
    f = FactoryBot.build(:filter, :search => '', :unlimited => '1', :organization_ids => [])
    assert f.valid?
    assert f.unlimited?
  ...
  test "filter remains set to unlimited when no taxonomy assigned and has empty search" do
    f = FactoryBot.build(:filter, :search => '', :unlimited => '0', :organization_ids => [],
                      :location_ids => [])
    assert f.valid?
    assert f.unlimited?
    assert_empty f.taxonomy_search
  ...
  test "search string composition" do
    f = FactoryBot.build :filter, :search => nil, :taxonomy_search => nil
    assert_equal '', f.search_condition

    f = FactoryBot.build :filter, :search => 'domain ~ test*', :taxonomy_search => nil
    assert_equal 'domain ~ test*', f.search_condition

    f = FactoryBot.build :filter, :search => nil, :taxonomy_search => 'organization_id = 1'
    assert_equal 'organization_id = 1', f.search_condition

    f = FactoryBot.build :filter, :search => 'domain ~ test*', :taxonomy_search => 'organization_id = 1'
    assert_equal '(domain ~ test*) and (organization_id = 1)', f.search_condition
  end

  test "filter with a valid search string is valid" do
    f = FactoryBot.build(:filter, :search => "name = 'blah'", :resource_type => 'Domain')
    assert_valid f
  end

  test "filter with an invalid search string is invalid" do
    f = FactoryBot.build(:filter, :search => "non_existent = 'blah'", :resource_type => 'Domain')
    refute_valid f
  end

  test "filter with an empty search string is valid" do
    f = FactoryBot.build(:filter, :search => nil, :resource_type => 'Domain')
    assert_valid f
  end
