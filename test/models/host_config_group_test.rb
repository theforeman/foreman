...
class HostConfigGroupTest < ActiveSupport::TestCase
  ...
  test 'relationship host.group_puppetclasses' do
    ...
    assert_equal %w[auth chkmk nagios pam].sort, host.group_puppetclasses.pluck(:name).sort
  end
  ...
  test 'relationship hostgroup.group_puppetclasses' do
    ...
    assert_equal %w[chkmk nagios git vim].sort, hostgroup.group_puppetclasses.pluck(:name).sort
  end

  test 'relationship hostgroup.config_groups' do
    hostgroup = hostgroups(:common)
    assert_equal 2, hostgroup.config_groups.count
    assert_equal %w[Monitoring Tools].sort, hostgroup.config_groups.pluck(:name).sort
  end
end
