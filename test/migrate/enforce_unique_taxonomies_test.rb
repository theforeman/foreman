...
class EnforceUniqueTaxonomiesTest < ActiveSupport::TestCase
  ...
  context 'up' do
    ...
    test 'index allows the same name for a location and an organization' do
      FactoryBot.create(:location, :name => 'shared-name')
      migrate_up

      assert TaxonomyRecord.create!(:type => 'Organization', :name => 'shared-name').persisted?
    end
  ...
  context 'renaming pre-existing duplicates' do
    ...
    test 'renames duplicate organizations keeping the oldest' do
      original = FactoryBot.create(:organization, :name => 'dupe-org')
      duplicate = create_duplicate_of(original)

      migrate_up

      assert_equal 'dupe-org', reload(original).name
      assert_equal "dupe-org-#{duplicate.id}", reload(duplicate).name
    end

    test 'does not modify the label column when renaming duplicates' do
      original = FactoryBot.create(:organization, :name => 'label-safe')
      duplicate = create_duplicate_of(original)
      original_label = reload(original).read_attribute(:label)
      duplicate_label = reload(duplicate).read_attribute(:label)

      migrate_up

      assert_equal original_label, reload(original).read_attribute(:label)
      assert_equal duplicate_label, reload(duplicate).read_attribute(:label)
    end

    test 'renames case-only duplicate organizations' do
      original = FactoryBot.create(:organization, :name => 'acme')
      duplicate = create_duplicate_of(original, :name => 'ACME')

      migrate_up

      assert_equal 'acme', reload(original).name
      assert_equal "ACME-#{duplicate.id}", reload(duplicate).name
    end

    test 'preserves label on every renamed organization in a group' do
      original = FactoryBot.create(:organization, :name => 'multi')
      second = create_duplicate_of(original)
      third = create_duplicate_of(original)
      labels_before = [original, second, third].map { |o| reload(o).read_attribute(:label) }

      migrate_up

      labels_after = [original, second, third].map { |o| reload(o).read_attribute(:label) }
      assert_equal labels_before, labels_after
    end
  end
  ...
  context 'edge cases' do
    test 'handles rows with nil type without crashing' do
      TaxonomyRecord.create!(:type => nil, :name => 'orphan', :title => 'orphan')
      original = FactoryBot.create(:location, :name => 'dupe')
      create_duplicate_of(original)
    ...
    test 'migration is idempotent when re-applied after rollback' do
      original = FactoryBot.create(:location, :name => 'dupe')
      create_duplicate_of(original)
      migrate_up
      migrate_down
    ...
    test 'does not rename anything when there are no duplicates' do
      loc = FactoryBot.create(:location, :name => 'unique-loc')
      org = FactoryBot.create(:organization, :name => 'unique-org')
      before_loc = [reload(loc).name, reload(loc).title]
      before_org = [reload(org).name, reload(org).title]

      migrate_up

      assert_equal before_loc, [reload(loc).name, reload(loc).title]
      assert_equal before_org, [reload(org).name, reload(org).title]
    end
  end
