...
class HostTest < ActiveSupport::TestCase
 ...
 test "a system should retrieve its finish template if it is associated to the correct environment only" do
  ...
  test 'fqdn of host with period in name returns just name with no concatination of domain' do
    host = hosts(:one)
    assert_equal "my5name.mydomain.net", host.name
    assert_equal host.name, host.fqdn
  end

  test 'fqdn of host without period in name returns name concatinated with domain' do
