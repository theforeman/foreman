...
class SettingTest < ActiveSupport::TestCase
  ...
  test "foreman_url must be a URI" do
    assert Setting.find_or_create_by(name: :name => "foreman_url", :default => "http://foo.com")
  ...
  test "unattended_url must be a URI" do
    assert Setting.find_or_create_by(name: :name => "unattended_url", :default => "http://foo.com")
  ...
  def check_frozen_change attr_name, value
    assert Setting.find_or_create_by(name: :name => "foo", :default => 5, :description => "test foo")
    setting = Setting.find_by_name("foo")
