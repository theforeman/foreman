...
class DnsTest < ActiveSupport::TestCase
  ...
  context "#lookup" do
    ...
    test "should do a forward ipv4 lookup" do
      Resolv::DNS.any_instance.expects(:getresource).with("www.example.com", Resolv::DNS::Resource::IN::CNAME).returns(nil)
    ...
    test "should do cname lookup" do
      resource_cname = mock()
      resource_cname.stubs(:name).returns("www.example.com")
      Resolv::DNS.any_instance.expects(:getresource).with("www.alias.com", Resolv::DNS::Resource::IN::CNAME).returns(resource_cname)

      resource_a = mock()
      resource_a.stubs(:address).returns("1.1.1.1")
      Resolv::DNS.any_instance.expects(:getresource).with("www.alias.com", Resolv::DNS::Resource::IN::A).returns(resource_a)

      result = Net::DNS.lookup("www.alias.com", :proxy => @proxy)
      assert_kind_of Net::DNS::CNAMERecord, result
      assert_equal "www.example.com", result.hostname
      assert_equal "www.alias.com", result.host_alias
    end
  end
  ...
  context "records" do
    ...
    context "a IPv4 reverse record" do
      ...
      test "should have a IPv4 forward record" do
        Resolv::DNS.any_instance.expects(:getresource).with("www.example.com", Resolv::DNS::Resource::IN::CNAME).returns(false)
    ...
    context "a IPv4 CNAME record" do
      setup do
        @record = Net::DNS::CNAMERecord.new(:hostname => "www.example.com", :ip => "01.02.03.004", :host_alias => "www.alias.com", :proxy => @proxy)
        Resolv::DNS.any_instance.expects(:getresource).with("www.example.com", Resolv::DNS::Resource::IN::CNAME).returns(nil)
      end
      ...
      test "should use IP from IPv4 record" do
        Resolv::DNS.any_instance.expects(:getresource).with("www.example.com", Resolv::DNS::Resource::IN::CNAME).returns(nil)
        assert_equal "1.2.3.4", @record.ip
      end
    end

    context "a IPv6 CNAME record" do
      setup do
        @record = Net::DNS::CNAMERecord.new(:hostname => "www.example.com", :ip => "2001:DB8::0000:1", :host_alias => "www.alias.com", :proxy => @proxy)
      end
      ...
      test "should use IPv6 if IPv4 not present" do
        Net::DNS::CNAMERecord.any_instance.expects(:a).returns(nil)
        assert_equal "2001:db8::1", @record.ip
      end

      test "#destroy calls delete on proxy with type parameter" do
        @proxy.expects(:delete).with("www.alias.com/CNAME")
        @record.destroy
      end
    end
  end
