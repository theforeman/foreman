...
module Rabl
  ...
  class Engine
    ...
    def data_name(data_token)
      # custom object root
      return params['root_name'] if self.respond_to?(:params) && params['root_name'].present? && !['false', false].include?(params['root_name'])
      # no object root for v2
      return nil if !self.respond_to?(:params) || api_version.to_i > 1 || ['false', false].include?(params['root_name'])
