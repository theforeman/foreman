...
We recommend using PostgreSQL 13+.
...
sudo dnf install gcc-c++ make git ruby ruby-devel rubygem-bundler \
    ...
    readline-devel systemd-devel tar libcurl-devel libcap-devel nodejs
....
...
For `libvirt-devel` you need the `crb` repository, for example on CentOS set the right `enabled=1` in `/etc/yum.repos.d/centos.repo`.

Then you also need to enable NodeJS 22:
...
[source, ruby]
....
bin/rake db:create # if not already created
bin/rake db:migrate
SEED_ADMIN_PASSWORD=changeme bin/rake db:seed
....
...
[source, ruby]
....
bin/rake permissions:reset password=changeme
....
...
[source, shell]
....
bin/rake test [TEST=<path_to_file>]
....

To run a specific test:
[source, shell]
....
bin/rails test <path_to_file>:<test_line_number>
....
...
bin/rake webpack:compile
bin/rake test TEST=test/integration/<test_file> [DEBUG_JS_TEST=1]
....
...
run `bin/rake db:migrate` and `bin/rake db:seed` to update the database scheme.
...
....
npm install # make sure to install npm dependencies for webpack
bin/rake webpack:compile # only needed if you have integration tests that uses JS
bin/rake test:<PLUGIN_NAME>
....

To run a specific plugin's test:
[source, ruby]
....
bin/rake test TEST="../<PLUGIN_PATH>/test/PATH/TO/TEST"
....
