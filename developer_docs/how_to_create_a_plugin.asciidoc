...
* `headerModifier` - optional PatternFly `<Th>` modifier (`wrap`, `truncate`, `nowrap`). When omitted, 1-word titles stay on one line, 2-word titles wrap, and 3+ word titles truncate with a tooltip. Set `truncate` for long titles such as Content view environments.
* `headerMaxWidth` - optional CSS max-width (for example `12ch`) so wrap/truncate can take effect. Defaults to `12ch` for wrap and `16ch` for truncate.
* `cellModifier` - optional PatternFly `<Td>` modifier (`wrap`, `truncate`, `nowrap`, `breakWord`). Defaults to `wrap`. Use `truncate` for long free-text and `breakWord` for values that have no spaces (FQDN, IPv6). Prefer CSS truncation with a PatternFly tooltip over character-count helpers such as `truncate(label, 35)`.
...
* You can use `bin/rake db:migrate` in your app directly to run all pending
migrations (from all available plugins).
* You can use `bin/rake db:migrate SCOPE=my_plugin` to apply migrations from a
single plugin only.
...
by running `bin/rake db:migrate SCOPE=my_plugin VERSION=0` statement.
...
[source, bash]
----
$ bin/rake plugin:assets:precompile[foreman_plugin]
----
...
$ bin/rake plugin:gettext[foreman_plugin]
$ bin/rake plugin:po_to_json[foreman_plugin]
----
...
[source, bash]
----
AS_DEPRECATION_WHITELIST=~/plugin_path AS_DEPRECATION_RECORD=yes bin/rake
