...
[[template-dirs]]
=== Registering plugin's directories with templates for database seed
(_Requires Foreman 3.10 or higher_)

By default, Foreman will look for the templates in the `:plugin/app/templates` directory.
If you want to add more directories, you can register them in your `engine.rb`:

[source, ruby]
....
Foreman::Plugin.register :my_plugin do
  # other code here
  register_template_dirs ["#{ForemanPlugin::Engine.root}/app/views/path/to/templates"]
end
....
