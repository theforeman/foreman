...
Custom validation methods allow you to run validations against other setting attributes like description or full_name, but please dont, use only `value`! :)

You have two ways to define the validations:
* inline with setting definition
...
There is special option `allow_blank`, that needs to be defined on setting definition.
This also allows a blank default value.

[source, ruby]
....
settings do
  category(:cfgmgmt) do
    # Following definitions are missing defaults and full names for simplification

    setting(:blank_setting, type: :string, description: 'Unnecessary setting', allow_blank: true)

    setting(:cool_setting, type: :string, description: 'Setting with only cool values', validate: /^cool.*/, allow_blank: true)

    setting(:cooler_puppet, type: :integer, description: 'Use Puppet that goes to 11', validate: ->(s) { s.value <= 11 })

    validates(:cooler_puppet, numericality: { greater_than: 11 }, allow_blank: true, if: -> { Setting[:cool_setting] == 'ultra_cool' })
