...
Declare Smart Proxy features in the plugin registration block. The declaration
creates the feature during database seeding and can list controller actions that
SSL-authenticated Smart Proxies providing the feature may access. Controller
names use the value returned by `controller_path`:
...
Foreman::Plugin.register :my_plugin do
  smart_proxy_feature 'My Feature',
    ...
    'my_plugin/api/v2/status' => :show
end
----

If the feature does not require access to a Foreman controller, omit the actions:
...
The controller-level `add_smart_proxy_filters` API remains available for
backwards compatibility, but new plugins should prefer the declaration above.
