...
Test helpers are available in `webpack/assets/javascripts/react_app/common/rtlTestHelpers.js`, or `foremanReact/common/rtlTestHelpers.js` in plugins:

* `renderWithStore(component, initialState)` — renders with a Redux `<Provider>`
* `renderWithI18n(component)` — renders with an i18n context wrapper
* `renderWithStoreAndI18n(component, initialState)` — combines both
